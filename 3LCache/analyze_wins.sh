...
if [ "$MODE" = "compare" ]; then
    ...
    [ -f "$CANDIDATES" ] || {
        HDR="trace,cache_size,start_win,end_win,support,mean_gain,mean_mab,dom_arm,w0,w1,w2,w3,write_ratio,avg_request_size,size_variance,singleton_ratio,popularity_skewness,avg_frequency,object_diversity,avg_reuse_distance,sequentiality_ratio,out_cache_hit_rate,request_rate,burstiness_index,arrival_time_variance,working_set_byte_delta,scan_ratio"
        for P in min max; do
            for F in write_ratio avg_request_size size_variance singleton_ratio \
                     ...
                     working_set_byte_delta scan_ratio; do
                HDR="${HDR},${P}_${F}"
            done
        done
        echo "$HDR" > "$CANDIDATES"
    ...
    awk -F, -v OFS=, \
        ...
        function emit_streak( i,dom,mx,v) {
            ...
            # Regime box: union of the per-window feature ranges in this streak.
            # Falls back to the streak-mean point if the run predates range logs.
            for (i=1; i<=15; i++) {
                printf ",%.9g",(bmin[i]=="" ? sums[i+4]/req_sum : bmin[i]) >> candidates
            }
            for (i=1; i<=15; i++) {
                printf ",%.9g",(bmax[i]=="" ? sums[i+4]/req_sum : bmax[i]) >> candidates
            }
            printf "\n" >> candidates
            confirmed++
            reset_streak()
        ...
        FNR==1 { reset_streak(); next }
        {
            ...
            if (is_win) {
                ...
                # Range columns (37..51 min, 52..66 max) exist only in v2 ledgers.
                if (NF >= 66) {
                    for (i=1; i<=15; i++) {
                        lo=$(36+i)+0; hi=$(51+i)+0
                        if (bmin[i]=="" || lo < bmin[i]) bmin[i]=lo
                        if (bmax[i]=="" || hi > bmax[i]) bmax[i]=hi
                    }
                }
            }
        }
    ...
    awk -F, -v OFS=, -v shrink="$SHRINK" \
        ...
        per_trace[$1] >= max_per_trace { next }
        {
            ...
            # WorkloadProfiler matrix formats:
            #   box row:   "box," + 15 min + 15 max + miss + weights
            #   point row: 15 features + miss + weights (legacy candidates)
            if (NF >= 57) {
                boxes++
                printf "box"
                for (i=28; i<=57; i++) printf OFS "%.9g",$i
            } else {
                points++
                for (i=13; i<=27; i++) {
                    if (i>13) printf OFS
        ...
        END {
            printf "  confirmed_policy_rows=%d (box=%d point=%d)\n", \
                   kept+0,boxes+0,points+0 > "/dev/stderr"
        }
